...
const PartnerProspects = ({ 
  ...
  }, [prospects, searchTerm, statusFilter, processFilter, partnerFilter, zoneFilter]);
  ...
  const handleSelectAll = () => {
    if (selectedCustomers.length === filteredProspects.length) {
      setSelectedCustomers([]);
    } else {
      setSelectedCustomers(filteredProspects.map(customer => customer.id));
    }
  };
  ...
  const handleBackToProspectList = () => {
  ...
  const stats = {
    ...
    active: prospects.filter(c => c.status === 'active').length,
    pending: prospects.filter(c => c.status === 'pending').length,
    inactive: prospects.filter(c => c.status === 'inactive').length,
    totalValue: prospects.reduce((sum, customer) => sum + customer.value, 0),
    prospect: prospects.filter(c => c.process === 'prospect').length,
    demo: prospects.filter(c => c.process === 'demo').length,
    poc: prospects.filter(c => c.process === 'poc').length,
    negotiating: prospects.filter(c => c.process === 'negotiating').length,
    lost: prospects.filter(c => c.process === 'lost').length
  };
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <Button
            variant="outline"
            onClick={handleBackToProspectList}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">Partner Prospects</h2>
          <p className="text-muted-foreground">Manage your sales pipeline and prospect relationships</p>
        </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>
              Prospects ({filteredProspects.length} of {prospects.length})
            </CardTitle>
            ...
            <TableHeader>
              <TableRow>
                <TableHead className="w-12">
                  <Checkbox 
                    checked={selectedCustomers.length === filteredProspects.length && filteredProspects.length > 0}
